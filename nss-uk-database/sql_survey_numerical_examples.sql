...
SELECT subject, SUM(response) FROM nss
 WHERE question='Q22' AND subject IN 
...
SELECT institution, ROUND(SUM(score*response)/SUM(response)) AS score FROM nss
...
SELECT institution, sum(sample), 
   SUM(CASE WHEN subject = '(8) Computer Science' THEN sample ELSE 0 END) FROM nss
